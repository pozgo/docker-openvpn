...
1. Create a client specific configuration:
        ...
        $ echo "ifconfig-push 192.168.254.1 192.168.254.2" | docker run -v $OVPN_DATA:/etc/openvpn -i --rm polinux/openvpn tee /etc/openvpn/ccd/CERT_COMMON_NAME
...
Login to the data volume with a `bash` container, note only changes in /etc/openvpn will persist:
    ...
    docker run -v $OVPN_DATA:/etc/openvpn -it --rm polinux/openvpn bash -l
...
If you're running an old configuration and need to upgrade it to pull in the ccd directory run the following:
    ...
    docker run  -v $OVPN_DATA:/etc/openvpn --rm polinux/openvpn ovpn_genconfig
