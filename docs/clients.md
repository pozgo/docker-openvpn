...
See an overview of the configured clients, including revocation status:
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn polinux/openvpn ovpn_listclients
...
Execute the following to generate the configuration for all clients:
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn --volume /tmp/openvpn_clients:/etc/openvpn/clients polinux/openvpn ovpn_getclient_all
...
Revoke `client1`'s certificate and generate the certificate revocation list (CRL):
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn polinux/openvpn easyrsa revoke client1
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn polinux/openvpn easyrsa gen-crl
