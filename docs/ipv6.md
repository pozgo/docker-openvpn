...
Copy the systemd init file from the docker-openvpn /init directory of the repository and install into `/etc/systemd/system/docker-openvpn.service`
    ...
    curl -o /etc/systemd/system/docker-openvpn@.service 'https://raw.githubusercontent.com/polinux/docker-openvpn/dev/init/docker-openvpn%40.service'
...
Generate server configuration with the udp6 or tcp6 protocol:
    ...
    docker run -v $OVPN_DATA:/etc/openvpn --rm polinux/openvpn ovpn_genconfig -u udp6://VPN.SERVERNAME.COM
    docker run -v $OVPN_DATA:/etc/openvpn --rm polinux/openvpn ovpn_genconfig -u tcp6://VPN.SERVERNAME.COM
